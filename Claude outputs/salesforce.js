...
function criarSalesforce(instanceUrl, accessToken, { https = require('https'), versao = VERSAO_PADRAO } = {}) {
  ...
  return {
    ...
    async itensDeFeed(parentId, { limite = 50 } = {}) {
      const r = await chamar('GET', `${dados}/chatter/feed-elements?q=&filterCommunityId=&feedElementType=FeedItem&subjectId=${encodeURIComponent(parentId)}&pageSize=${limite}`);
      return r.elements || [];
    },
...
module.exports = { criarSalesforce, idDoLink, escaparSoql, mascararSegredos, SalesforceErro, VERSAO_PADRAO };
