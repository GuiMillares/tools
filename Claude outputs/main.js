...
ipcMain.handle('salesforce:diagnostico', async (event, { dominioExemplo } = {}) => {
  const log = [];
  const push = (message, type = 'info') => log.push({ message, type });
  try {
    const resumo = await sfComSessao(async (sf) => {
      ...
      const personalizados = (d.fields || []).filter((f) => f.custom).map((f) => `${f.label} [${f.name}]${f.createable && !f.nillable ? ' OBRIGATÓRIO' : ''}`);
      ...
      if (alvo) {
        ...
        const tarefas = await sf.consultar(
          `SELECT Id, Subject, Status, OwnerId, Owner.Name, WhatId, What.Name, What.Type, WhoId, Who.Name, CreatedById, CreatedBy.Name, CreatedDate, Description ` +
          `FROM Task WHERE Subject LIKE '%${escaparSoql(alvo)}%' ORDER BY CreatedDate DESC LIMIT 3`
        );
        push(`${tarefas.length} tarefa(s) encontrada(s).`, tarefas.length ? 'success' : 'warn');
        for (const t of tarefas) {
          push(`  ${t.Subject} | status ${t.Status} | dono ${t.Owner?.Name} | criada por ${t.CreatedBy?.Name} | relativo a ${t.What?.Type} ${t.What?.Name || t.WhatId || '(nada)'}`, 'info');
          if (t.Description) push(`  Comentários: ${mascararSegredos(t.Description).slice(0, 200)}`, 'info');
        }
        exemplo = tarefas[0] || null;

        if (exemplo?.WhatId) {
          push(`GET itens do feed do caso ${exemplo.WhatId}`, 'cmd');
          try {
            const itens = await sf.itensDeFeed(exemplo.WhatId, { limite: 25 });
            push(`${itens.length} item(ns) no feed.`, 'info');
            for (const it of itens.slice(0, 8)) {
              const texto = (it.body?.text || '').replace(/\s+/g, ' ').slice(0, 70);
              push(`  [${it.id}] tipo ${it.type} | de ${it.actor?.name || '?'} | ${texto}`, 'info');
            }
          } catch (e) {
            push(`Não consegui ler o feed: ${e.message}`, 'warn');
          }
        }
      }
      ...
      return {
        ...
        exemplo: exemplo ? { id: exemplo.Id, assunto: exemplo.Subject, casoId: exemplo.WhatId, contatoId: exemplo.WhoId } : null,
      };
    });
